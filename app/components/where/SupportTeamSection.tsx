...
export default function SupportTeamSection({
  ...
  const teamMembers = [];
  if (teamData.hub) teamMembers.push({ 
    ...teamData.hub, 
    type: "technical", 
    role: t.hubRole,
  ...
  if (teamData.distributor) teamMembers.push({ 
  ...
  if (teamData.lab) teamMembers.push({ 
    ...teamData.lab, 
    type: "lab", 
    role: t.labRole,
    ...
    <div className="mt-32 w-full animate-in fade-in slide-in-from-bottom-6 duration-1000 relative z-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-24 w-full">
        {teamMembers.map((member, idx) => (
          <div key={idx} className="flex flex-col items-center md:items-start text-center md:text-left group flex-grow h-full bg-white relative z-10">
             ...
             <h3 className="text-xl md:text-2xl font-black text-[#111111] mb-1 tracking-tight">
               {member.name}
             </h3>
             
             {/* NOMBRES DE LAS INSTITUCIONES (MÁS GRANDES) */}
             <span className="text-lg md:text-xl font-bold text-gray-800 mb-4 block italic">
               {member.entityName}
             </span>
      ...
      >
        ...
        <div className="w-full lg:w-5/12 relative z-10 text-center lg:text-left">
           <h3 className="text-3xl md:text-5xl font-black text-white mb-6 tracking-tighter leading-[1.1]">
             {t.needHelpTitle}
           </h3>
